...
(() => {
  ...
  const toast = msg => {
    ...
    clearTimeout(window.__dpExcelToast); window.__dpExcelToast = setTimeout(() => el.classList.remove('show'), 3500);
  };

  function clean(v) { return v == null ? '' : String(v).trim(); }
  function normalize(v) { return clean(v).toLowerCase().replace(/[.]/g,'').replace(/\s+/g,' '); }
  ...
  async function getSchoolId() {
    ...
    if (error || !data?.school_id) throw new Error('Sekolah akun belum ditemukan.');
  ...
  function parseSheet(ws, className) {
    ...
    const indexes = {};
    expected.forEach(h => { indexes[h] = headers.findIndex(x => x === h); });
    if (indexes.nama < 0) throw new Error(`Sheet "${className}": kolom Nama tidak ditemukan pada baris 2.`);
    const students = [];
    for (let r = 2; r < rows.length; r++) {
      const row = rows[r] || [];
      const name = cell(row, indexes.nama);
      if (!name) continue;
      students.push({
        nis: indexes.nis >= 0 ? cell(row, indexes.nis) : '',
        name,
        class_name: className,
        homeroom_teacher: homeroom,
        parent_name: indexes['nama orang tua'] >= 0 ? cell(row, indexes['nama orang tua']) : '',
  ...
  async function importWorkbook(file) {
    ...
    const all = [];
    for (const className of wb.SheetNames) all.push(...parseSheet(wb.Sheets[className], className));

    // NIS is required by the current DB schema. For rows without NIS, generate a stable import placeholder.
    const used = new Set();
    const payload = all.map((s, i) => {
      let nis = s.nis;
      if (!nis) nis = `IMP-${Date.now()}-${i + 1}`;
      if (used.has(nis)) nis = `${nis}-${i + 1}`;
      used.add(nis);
      return { school_id: schoolId, nis, name: s.name, class_name: s.class_name, homeroom_teacher: s.homeroom_teacher || null, parent_name: s.parent_name || null, parent_phone: s.parent_phone || null, notes: s.notes || null, status: 'active' };
    });
  ...
  function install() {
    document.addEventListener('change', async e => {
      ...
      if (!input || input.id !== 'excelStudentImport') return;
      const file = input.files?.[0]; if (!file) return;
      if (!/\.(xlsx|xls)$/i.test(file.name)) { toast('Pilih file Excel .xlsx atau .xls.'); input.value=''; return; }
      try {
        toast('Membaca Excel dan memproses semua sheet...');
        const result = await importWorkbook(file);
        toast(`Berhasil: ${result.inserted} siswa dari ${result.sheets} kelas diimpor dan berstatus aktif.`);
        input.value = '';
        document.dispatchEvent(new CustomEvent('dp:students-imported', { detail: result }));
      } catch (err) {
        console.error('[Disiplin Pro Excel]', err);
        toast(err?.message || 'Import Excel gagal.');
        input.value = '';
      }
    });
  }
